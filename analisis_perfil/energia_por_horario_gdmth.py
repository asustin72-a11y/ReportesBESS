"""
Estadisticas de energia por horario tarifario CFE GDMTH (Region Central).

Lee perfiles cincominutales (FECHA, KWH_REC, ...) y genera:
  - CSV diario:  FECHA, BASE_REC, INTERMEDIO_REC, PUNTA_REC, TOTAL_REC
  - CSV mensual: ANIO, MES, BASE_REC, INTERMEDIO_REC, PUNTA_REC, TOTAL_REC

Dia operativo: 00:05 del dia D hasta 00:00 del dia D+1 (inclusive).
Script 100 % autonomo (sin dependencias del repo BESS).
"""
...
# ---------------------------------------------------------------------------
# Reglas GDMTH Region Central (equivalentes a bess/cfe/periods_gdmth.py)
# ---------------------------------------------------------------------------
...
def obtener_temporada_gdmth(fecha: date) -> int:
    ...
    anio = fecha.year
    inicio_verano = _primer_domingo_abril(anio)
    fin_verano = _ultimo_domingo_octubre(anio) - timedelta(days=1)
...
def obtener_periodo_gdmth_por_hora(fecha: date, hora: int) -> str:
    """Clasifica la hora CFE (0-23) segun tablas GDMTH Central."""
...
def fecha_operativa(dt: datetime) -> date:
    """Dia D = 00:05 D ... 00:00 D+1 (el 00:00 cuenta para el dia anterior)."""
...
def _es_reporte(nombre: str) -> bool:
    return (
        nombre.endswith("_energia_por_dia.csv")
        or nombre.endswith("_energia_por_mes.csv")
        or nombre.endswith("_gdmth_energia_por_dia.csv")
        or nombre.endswith("_gdmth_energia_por_mes.csv")
    )
...
def rutas_salida(entrada: Path) -> tuple[Path, Path]:
    stem = entrada.stem
    return (
        entrada.with_name(f"{stem}_gdmth_energia_por_dia.csv"),
        entrada.with_name(f"{stem}_gdmth_energia_por_mes.csv"),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    for entrada in entradas:
        print(f"\nProcesando (GDMTH, {servicio}): {entrada.name}")
